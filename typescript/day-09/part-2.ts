...
const takeFirsts = (layers: number[][]): number[] => layers.map(layer => layer[0]);
const firsts = layers.map(takeFirsts);

const results = firsts.map(difs => difs.reverse().reduce((acc, num) => num - acc, 0));
